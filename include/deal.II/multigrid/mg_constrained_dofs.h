...
public:
   ...
   * get_level_constraint_matrix(level). Note that the current functionality
   * periodicity constraints does not support rotation matrices in the
...
private:
  ...
  /**
   * Constraint matrices for each level.
   */
...
inline
const ConstraintMatrix &
MGConstrainedDoFs::get_level_constraint_matrix (unsigned int level) const
{
